...
{
public:
  LogicalSwitchEditWindow(uint8_t ls) : Page(),
                                        ls(ls)
  {
    buildBody(&body);
    buildHeader(&header);
...
{
public:
  LogicalSwitchButton(Window *parent, const rect_t &rect, int lsIndex, std::function<uint8_t(void)> onPress) : Button(parent, rect, onPress),
                                                                                                               lsIndex(lsIndex),
                                                                                                               active(isActive())
  {
...
ModelLogicalSwitchesPage::ModelLogicalSwitchesPage() : PageTab(STR_MENULOGICALSWITCHES, ICON_MODEL_LOGICAL_SWITCHES)
{
}
