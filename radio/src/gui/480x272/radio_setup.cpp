...
class DateTimeWindow : public Window {
 public:
  DateTimeWindow(Window *parent, const rect_t &rect) : Window(parent, rect) {
    build();
  }

  ~DateTimeWindow() { deleteChildren(); }
 ...
 protected:
  ...
  void build() {
    ...
    new NumberEdit(
        this, grid.getFieldSlot(3, 0), 2018, 2100,
        [=]() -> int32_t {
          struct gtm t;
          gettime(&t);
    ...
    auto month = new NumberEdit(
        this, grid.getFieldSlot(3, 1), 1, 12,
        [=]() -> int32_t {
          struct gtm t;
          gettime(&t);
          return 1 + t.tm_mon;
    ...
    int8_t dlim = (((((year%4==0) && (year%100!=0)) || (year%400==0)) &&
    (t.tm_mon==1)) ? 1 : 0); static const pm_uint8_t dmon[] PROGMEM = { 31, 28,
    31, 30, 31, 30, 31, 31, 30, 31, 30, 31 }; dlim +=
    pgm_read_byte(&dmon[t.tm_mon]);*/
    int8_t dlim = 31;
    auto day = new NumberEdit(
        this, grid.getFieldSlot(3, 2), 1, dlim,
        [=]() -> int32_t {
          struct gtm t;
          gettime(&t);
          return t.tm_mday;
    ...
    auto hour = new NumberEdit(
        this, grid.getFieldSlot(3, 0), 0, 24,
        [=]() -> int32_t {
          struct gtm t;
          gettime(&t);
          return t.tm_hour;
    ...
    auto minutes = new NumberEdit(
        this, grid.getFieldSlot(3, 1), 0, 59,
        [=]() -> int32_t {
          struct gtm t;
          gettime(&t);
          return t.tm_min;
    ...
    auto seconds = new NumberEdit(
        this, grid.getFieldSlot(3, 2), 0, 59,
        [=]() -> int32_t {
          struct gtm t;
          gettime(&t);
          return t.tm_sec;
...
void RadioSetupPage::build(Window *window) {
  ...
  {
    grid.addWindow(
        new DateTimeWindow(window, {0, grid.getWindowHeight(), LCD_W, 0}));
  }

  // Batt meter range - Range 3.0v to 16v
  new StaticText(window, grid.getLabelSlot(), STR_BATTERY_RANGE);
  auto batMinEdit = new NumberEdit(
      window, grid.getFieldSlot(2, 0), -60 + 90, g_eeGeneral.vBatMax + 29 + 90,
      GET_SET_WITH_OFFSET(g_eeGeneral.vBatMin, 90), PREC1);
  batMinEdit->setSuffix("V");
  auto batMaxEdit = new NumberEdit(
      window, grid.getFieldSlot(2, 1), g_eeGeneral.vBatMin - 29 + 120, 40 + 120,
  ...
  new Slider(window, grid.getFieldSlot(), -VOLUME_LEVEL_DEF,
  ...
  new Slider(window, grid.getFieldSlot(), -2, +2,
  ...
  edit = new NumberEdit(
      window, grid.getFieldSlot(), VARIO_FREQUENCY_ZERO - 400,
      VARIO_FREQUENCY_ZERO + 400,
...
/*
  case ITEM_SETUP_VARIO_RANGE:
    ...
    lcdDrawNumber(RADIO_SETUP_2ND_COLUMN, y,
  VARIO_FREQUENCY_ZERO+(g_eeGeneral.varioPitch*10)+VARIO_FREQUENCY_RANGE+(g_eeGeneral.varioRange*10),
  attr|LEFT, 0, NULL, "Hz"); if (attr) CHECK_INCDEC_GENVAR(event,
  g_eeGeneral.varioRange, -80, 80); break; case ITEM_SETUP_VARIO_REPEAT:
    lcdDrawText(MENUS_MARGIN_LEFT, y, STR_REPEAT_AT_ZERO);
    lcdDrawNumber(RADIO_SETUP_2ND_COLUMN, y,
  VARIO_REPEAT_ZERO+(g_eeGeneral.varioRepeat*10), attr|LEFT, 0, NULL, STR_MS);
...
*/
...

  {
    ...
    edit = new NumberEdit(window, grid.getFieldSlot(), 30, 120,
    ...
    edit = new NumberEdit(window, grid.getFieldSlot(), 0, 250,
...

  {
    ...
    // Backlight mode
    new StaticText(window, grid.getLabelSlot(true), STR_MODE);
    new Choice(window, grid.getFieldSlot(), STR_VBLMODE, e_backlight_mode_off,
               e_backlight_mode_on, GET_SET_DEFAULT(g_eeGeneral.backlightMode));
    grid.nextLine();

    // Delay
    new StaticText(window, grid.getLabelSlot(true), STR_BLDELAY);
    auto edit =
        new NumberEdit(window, grid.getFieldSlot(2, 0), 15, 600,
    ...
    new Slider(
        window, grid.getFieldSlot(), BACKLIGHT_LEVEL_MIN, BACKLIGHT_LEVEL_MAX,
        [=]() -> int32_t {
    ...
    new Slider(window, grid.getFieldSlot(), BACKLIGHT_LEVEL_MIN,
               BACKLIGHT_LEVEL_MAX, GET_SET_DEFAULT(g_eeGeneral.blOffBright));
    grid.nextLine();

    // Flash beep
    new StaticText(window, grid.getLabelSlot(true), STR_ALARM);
    new CheckBox(window, grid.getFieldSlot(),
                 GET_SET_DEFAULT(g_eeGeneral.alarmsFlash));
    grid.nextLine();
  }

  {
    ...
    new NumberEdit(window, grid.getFieldSlot(2, 0), -12, 12,
                   GET_SET_DEFAULT(g_eeGeneral.timezone));
    grid.nextLine();

    // Adjust RTC (from telemetry)
    new StaticText(window, grid.getLabelSlot(true), STR_ADJUST_RTC);
    new CheckBox(window, grid.getFieldSlot(),
                 GET_SET_DEFAULT(g_eeGeneral.adjustRTC));
    grid.nextLine();

    // GPS format
    new StaticText(window, grid.getLabelSlot(true), STR_GPSCOORD);
    new Choice(window, grid.getFieldSlot(), STR_GPSFORMAT, 0, 1,
  ...
  new Choice(window, grid.getFieldSlot(), STR_COUNTRYCODES, 0, 2,
  ...
  auto choice = new Choice(
      window, grid.getFieldSlot(), nullptr, 0, DIM(languagePacks) - 2,
  ...
  new Choice(window, grid.getFieldSlot(), STR_VUNITSSYSTEM, 0, 1,
  ...
  case ITEM_SETUP_FAI:
    ...
    if (g_eeGeneral.fai) {
      ...
      g_eeGeneral.fai =
          editCheckBox(g_eeGeneral.fai, RADIO_SETUP_2ND_COLUMN, y, attr, event);
    ...
    new Choice(window, grid.getFieldSlot(), STR_MAVLINK_BAUDS, 0, 7,
               GET_SET_DEFAULT(g_eeGeneral.mavbaud));
    grid.nextLine();
#endif
    ...
    edit = new NumberEdit(
        window, grid.getFieldSlot(2, 0), -15, 100 - 15,
    ...
    choice = new Choice(window, grid.getFieldSlot(), STR_USBMODES,
                        USB_UNSELECTED_MODE, USB_MAX_MODE,
    ...
    */
    ...
    new StaticText(window, grid.getLabelSlot(),
                   STR_RXCHANNELORD);  // RAET->AETR
    choice = new Choice(window, grid.getFieldSlot(), nullptr, 0, 4 * 3 * 2 - 1,
    ...
    choice =
        new Choice(window, grid.getFieldSlot(), nullptr, 0, 3,
                   GET_DEFAULT(g_eeGeneral.stickMode), [=](uint8_t newValue) {
                     pausePulses();
    ...
    choice->setTextHandler([](uint8_t value) {
      return std::to_string(1 + value) + ": left=" +
             std::string(
                 &getSourceString(MIXSRC_Rud + modn12x3[4 * value])[0]) +
             "+" +
             std::string(
