...
class VarioChoice : public SourceChoice {
 public:
  VarioChoice(Window *window, const rect_t &rect,
              IsValueAvailable isValueAvailable)
      : SourceChoice(window, rect, MIXSRC_NONE, MIXSRC_LAST_TELEM,
                     GET_DEFAULT(g_model.frsky.varioSource
                                     ? MIXSRC_FIRST_TELEM +
                                           3 * (g_model.frsky.varioSource - 1)
                                     : MIXSRC_NONE),
                     SET_VALUE(g_model.frsky.varioSource,
...
class SensorButton : public Button {
 public:
  ...
  void paint(BitmapBuffer *dc) override {
    ...
    lcdDrawSizedText(SENSOR_COL1, line1, g_model.telemetrySensors[index].label,
                     TELEM_LABEL_LEN, ZCHAR);

    if (telemetryItem.isAvailable()) {
      LcdFlags flags =
          LEFT | (telemetryItem.isOld() ? ALARM_COLOR : TEXT_COLOR);
      if (sensor.unit == UNIT_GPS) flags |= EXPANDED;
      drawSensorCustomValue(SENSOR_COL2, line1, index,
                            getValue(MIXSRC_FIRST_TELEM + 3 * index), flags);
    } else {
...
class SensorEditWindow : public Page {
 public:
  SensorEditWindow(uint8_t index) : Page(), index(index) {
    buildBody(&body);
    buildHeader(&header);
  }

  void checkEvents() { invalidate(); }

  ~SensorEditWindow() { body.deleteChildren(); }
 ...
 protected:
  ...
  void buildHeader(Window *window) {
    ...
    if (unit != UNIT_RAW) {
      char unitStr[8];
      strAppend(unitStr, STR_VTELEMUNIT + 1 + unit * STR_VTELEMUNIT[0],
                STR_VTELEMUNIT[0]);
      new StaticText(
          window, {185, 4, 200, 20},
  ...
  void updateSensorOneWindow() {
    ...
    if (sensor->type == TELEM_TYPE_CALCULATED) {
      ...
      new Choice(sensorOneWindow, grid.getFieldSlot(), STR_VFORMULAS, 0,
      ...
      auto hex = new NumberEdit(sensorOneWindow, grid.getFieldSlot(2, 0), 0,
                                0xffff, GET_SET_DEFAULT(sensor->id));
      hex->setDisplayHandler(
          [](BitmapBuffer *dc, LcdFlags flags, int32_t value) {
            lcdDrawHexNumber(2, 2, value, 0);
          });
      new NumberEdit(sensorOneWindow, grid.getFieldSlot(2, 1), 0, 0xff,
    ...
    if ((sensor->type == TELEM_TYPE_CALCULATED &&
      ...
      new Choice(sensorOneWindow, grid.getFieldSlot(), STR_VTELEMUNIT, 0,
                 UNIT_MAX, GET_DEFAULT(sensor->unit), [=](uint8_t newValue) {
    ...
    if (sensor->isPrecConfigurable() && sensor->unit != UNIT_FAHRENHEIT) {
      ...
      new Choice(sensorOneWindow, grid.getFieldSlot(), STR_VPREC, 0, 2,
    ...
    if (sensor->unit < UNIT_FIRST_VIRTUAL) {
      if (sensor->type == TELEM_TYPE_CALCULATED) {
        if (sensor->formula == TELEM_FORMULA_CELL) {
          ...
          new SensorSourceChoice(sensorOneWindow, grid.getFieldSlot(),
                                 &sensor->dist.gps, isGPSSensor);
        } else if (sensor->formula == TELEM_FORMULA_CONSUMPTION) {
          new StaticText(sensorOneWindow, grid.getLabelSlot(),
                         STR_CURRENTSENSOR);
          new SensorSourceChoice(sensorOneWindow, grid.getFieldSlot(),
                                 &sensor->consumption.source,
                                 isSensorAvailable);
        ...
        if (sensor->unit == UNIT_RPMS) {
          ...
          new NumberEdit(sensorOneWindow, grid.getFieldSlot(), 1, 30000,
                         GET_SET_DEFAULT(sensor->custom.ratio));
        } else {
          new StaticText(sensorOneWindow, grid.getLabelSlot(), STR_RATIO);
          auto edit =
              new NumberEdit(sensorOneWindow, grid.getFieldSlot(), 0, 30000,
                             GET_SET_DEFAULT(sensor->custom.ratio));
          edit->setZeroText("-");
        }
      }
    ...
    if (!(sensor->unit == UNIT_GPS || sensor->unit == UNIT_DATETIME ||
      ...
      if (sensor->type == TELEM_TYPE_CALCULATED) {
        if (sensor->formula == TELEM_FORMULA_CELL) {
          ...
          new Choice(sensorOneWindow, grid.getFieldSlot(), STR_VCELLINDEX, 0, 8,
        ...
        new NumberEdit(
            sensorOneWindow, grid.getFieldSlot(), -30000, 30000,
            GET_SET_DEFAULT(sensor->custom.offset),
    ...
    if (sensor->unit != UNIT_RPMS && sensor->isConfigurable()) {
      ...
      new CheckBox(sensorOneWindow, grid.getFieldSlot(),
    ...
    new CheckBox(sensorOneWindow, grid.getFieldSlot(),
                 GET_DEFAULT(sensor->logs), [=](int32_t newValue) {
  ...
  void buildBody(Window *window) {
    ...
    new Choice(window, grid.getFieldSlot(), STR_VSENSORTYPES, 0, 1,
...
void ModelTelemetryPage::build(Window *window, int8_t focusSensorIndex) {
  ...
  auto edit = new NumberEdit(window, grid.getFieldSlot(), -30, 30,
                             GET_SET_DEFAULT(g_model.rssiAlarms.warning));
  ...
  edit = new NumberEdit(window, grid.getFieldSlot(), -30, 30,
                        GET_SET_DEFAULT(g_model.rssiAlarms.critical));
  edit->setDisplayHandler([](BitmapBuffer *dc, LcdFlags flags, int32_t value) {
  ...
  new CheckBox(window, grid.getFieldSlot(),
               GET_SET_DEFAULT(g_model.rssiAlarms.disabled));
  grid.nextLine();
  ...
  new StaticText(window,
                 {SENSOR_COL2, grid.getWindowHeight() + 3,
                  SENSOR_COL3 - SENSOR_COL2, lineHeight},
                 STR_VALUE, SMLSIZE | TEXT_DISABLE_COLOR);
  if (!g_model.ignoreSensorIds && !IS_SPEKTRUM_PROTOCOL())
    new StaticText(window,
                   {SENSOR_COL3, grid.getWindowHeight() + 3,
                    LCD_W - SENSOR_COL3, lineHeight},
                   STR_ID, SMLSIZE | TEXT_DISABLE_COLOR);
  grid.nextLine();
  ...
  new TextButton(
      window, grid.getLineSlot(), STR_DELETE_ALL_SENSORS, []() -> uint8_t {
  ...
  new CheckBox(window, grid.getFieldSlot(),
               GET_SET_DEFAULT(g_model.ignoreSensorIds));
  grid.nextLine();
 ...
 auto choice = new SourceChoice(window, grid.getFieldSlot(), MIXSRC_NONE, MIXSRC_LAST_TELEM,
  ...
  new NumberEdit(window, grid.getFieldSlot(2, 0), -7, 7,
                 GET_SET_WITH_OFFSET(g_model.frsky.varioMin, -10));
  new NumberEdit(window, grid.getFieldSlot(2, 1), -7, 7,
                 GET_SET_WITH_OFFSET(g_model.frsky.varioMax, 10));
  grid.nextLine();
  new StaticText(window, grid.getLabelSlot(true), STR_CENTER);
  new NumberEdit(window, grid.getFieldSlot(3, 0), -7, 7,
                 GET_SET_WITH_OFFSET(g_model.frsky.varioCenterMin, -5), PREC1);
  new NumberEdit(window, grid.getFieldSlot(3, 1), -7, 7,
                 GET_SET_WITH_OFFSET(g_model.frsky.varioCenterMax, 5), PREC1);
  new Choice(window, grid.getFieldSlot(3, 2), STR_VVARIOCENTER, 0, 1,
             GET_SET_DEFAULT(g_model.frsky.varioCenterSilent));
  grid.nextLine();
