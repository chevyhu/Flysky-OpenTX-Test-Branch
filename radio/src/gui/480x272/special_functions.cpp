  ...
  void updateSpecialFunctionOneWindow() {
    ...
    switch (func) {
      ...
      case FUNC_TRAINER:
        new StaticText(specialFunctionOneWindow, grid.getLabelSlot(),
                       STR_TIMER);
        new NumberEdit(specialFunctionOneWindow, grid.getFieldSlot(), 1, 3,
      ...
      case FUNC_RESET:
        if (CFN_PARAM(cfn) < FUNC_RESET_PARAM_FIRST_TELEM) {
          ...
          auto choice = new Choice(
              specialFunctionOneWindow, grid.getFieldSlot(), nullptr, 0,
      ...
      case FUNC_SET_TIMER: {
        new StaticText(specialFunctionOneWindow, grid.getLabelSlot(),
                       STR_TIMER);
        auto timerchoice =
            new Choice(specialFunctionOneWindow, grid.getFieldSlot(), nullptr,
                       0, TIMERS - 1, GET_SET_DEFAULT(CFN_TIMER_INDEX(cfn)));
      ...
      case FUNC_LOGS: {
        ...
        edit->setDisplayHandler(
            [=](BitmapBuffer *dc, LcdFlags flags, int32_t value) {
              lcdDrawNumber(2, 2, CFN_PARAM(cfn), PREC1, sizeof(CFN_PARAM(cfn)),
                            nullptr, "s");
            });
        break;
      }
      ...
      repeat->setDisplayHandler(
          [](BitmapBuffer *dc, LcdFlags flags, int32_t value) {
            if (value == 0)
              ...
              drawNumber(dc, 2, 2, value * CFN_PLAY_REPEAT_MUL, flags, 0,
                         nullptr, "s");
          });
    }
  }
  ...
  void buildBody(Window *window) {
    ...
    auto choice =
        new Choice(window, grid.getFieldSlot(), STR_VFSWFUNC, 0, FUNC_MAX - 1,
                   GET_DEFAULT(CFN_FUNC(cfn)), [=](int32_t newValue) {
    ...
    specialFunctionOneWindow =
        new Window(window, {0, grid.getWindowHeight(), LCD_W, 0});
    updateSpecialFunctionOneWindow();
    ...
    switch (func) {
      ...
      case FUNC_PLAY_SCRIPT:
        if (ZEXIST(cfn->play.name))
          lcdDrawSizedText(col1, line2, cfn->play.name, sizeof(cfn->play.name),
                           0);
        else
      ...
      case FUNC_SET_TIMER:
        drawStringWithIndex(col1, line2, STR_TIMER, CFN_TIMER_INDEX(cfn) + 1,
                            0);
        break;
      ...
      case FUNC_LOGS:
        lcdDrawNumber(col3, line1, CFN_PARAM(cfn), PREC1,
...
void SpecialFunctionsPage::rebuild(Window *window,
...
void SpecialFunctionsPage::build(Window *window, int8_t focusIndex) {
  ...
  for (uint8_t i = 0; i < MAX_SPECIAL_FUNCTIONS; i++) {
    ...
    if (focusIndex == i) button->setFocus();
    button->setPressHandler([=]() -> uint8_t {
      ...
      if (!CFN_EMPTY(cfn) && CFN_EMPTY(&functions[MAX_SPECIAL_FUNCTIONS - 1])) {
        menu->addLine(STR_INSERT, [=]() {
          memmove(cfn + 1, cfn,
                  (MAX_SPECIAL_FUNCTIONS - menuVerticalPosition - 1) *
