...
enum LogicalSwitchContextState { SWITCH_START, SWITCH_DELAY, SWITCH_ENABLE };
...
PACK(typedef struct { LogicalSwitchContext lsw[MAX_LOGICAL_SWITCHES]; })
...
#define CHECK_3POS(idx, sw) \
  newPos |= check3PosSwitchPosition(idx, sw##0, startup)
      ...
      if (IS_MULTIPOS_CALIBRATED(calib)) {
        ...
        if (startup) {
          ...
          if (previousStoredPos != pos) {
            PLAY_SWITCH_MOVED(SWSRC_LAST_SWITCH + i * XPOTS_MULTIPOS_COUNT +
                              pos);
          }
        }
      }
    ...
    if (s == LS_FAMILY_COMP) {
      ...
      if (v1 >= MIXSRC_FIRST_TELEM) {
        ...
        if ((!TELEMETRY_STREAMING() &&
             v1 >= MIXSRC_FIRST_TELEM + TELEM_FIRST_STREAMED_VALUE - 1) ||
            IS_FAI_FORBIDDEN(v1 - 1)) {
#endif
          result = false;
      ...
      switch (ls->func) {
        ...
        case LS_FUNC_ANEG:
          ...
          break;
        default: {
          if (LS_LAST_VALUE(mixerCurrentFlightMode, idx) ==
              CS_LAST_VALUE_INIT) {
  ...
  if (ls->delay || ls->duration) {
    ...
    if (result) {
      ...
      if (context.timerState == SWITCH_ENABLE) {
        result =
            (ls->duration == 0 ||
             context.timer > 0);  // return false after duration timer runs out
...
#endif
{
  ...
  if (swtch == SWSRC_NONE) return true;
  ...
  if (cs_idx == SWSRC_ONE) {
    ...
    if (startupWarningState < STARTUP_WARNING_DONE) {
      // if throttle or switch warning is currently active, ignore actual stick
      // position and use wanted values
      if (cs_idx <= 3) {
        if (!(g_model.switchWarningEnable &
              1)) {  // ID1 to ID3 is just one bit in switchWarningEnable
          result = (cs_idx) == ((g_model.switchWarningState & 3) +
                                1);  // overwrite result with desired value
        }
      } else if (!(g_model.switchWarningEnable & (1 << (cs_idx - 3)))) {
        // current switch should not be ignored for warning
        result = g_model.switchWarningState &
                 (1 << (cs_idx - 2));  // overwrite result with desired value
      }
    }
#endif
  }
...
swsrc_t getMovedSwitch() {
  ...
  for (uint8_t i = NUM_PSWITCH; i > 1; i--) {
    ...
    // don't use getSwitch here to always get the proper value, even getSwitch
    // manipulates
  ...
  if ((tmr10ms_t)(get_tmr10ms() - s_move_last_time) > 10) result = 0;
...
#undef GETADC_COUNT
    ...
    if (g_model.potsWarnMode) {
      ...
      for (int i = 0; i < NUM_POTS + NUM_SLIDERS; i++) {
        ...
        if (!(g_model.potsWarnEnabled & (1 << i)) &&
            (abs(g_model.potsWarnPosition[i] - GET_LOWRES_POT_POSITION(i)) >
             1)) {
          warn = true;
          bad_pots |= (1 << i);
        }
  ...
  for (int i = 0; i < NUM_SWITCHES - 1; i++) {
    if (!(g_model.switchWarningEnable & (1 << i))) {
      ...
      } else if ((states & (1 << (i + 1))) !=
                 (switches_states & (1 << (i + 1)))) {
        warn = true;
      }
    }
  }
    ...
    if ((last_bad_switches != switches_states) || (last_bad_pots != bad_pots)) {
      ...
      int x = SWITCH_WARNING_LIST_X - SWITCH_WARNING_LIST_X_OFFSET,
          y = SWITCH_WARNING_LIST_Y;
      int numWarnings = 0;
      ...
      if (g_model.potsWarnMode) {
        ...
        for (int i = 0; i < NUM_POTS + NUM_SLIDERS; i++) {
          ...
          if (!(g_model.potsWarnEnabled & (1 << i))) {
            if (abs(g_model.potsWarnPosition[i] - GET_LOWRES_POT_POSITION(i)) >
                1) {
#if defined(COLORLCD)
              char s[8];
              // TODO add an helper
              strncpy(s,
                      &STR_VSRCRAW[1 + (NUM_STICKS + 1 + i) * STR_VSRCRAW[0]],
                      STR_VSRCRAW[0]);
  ...
  if (last_bad_switches != switches_states) {
    RAISE_ALERT(STR_SWITCHWARN, NULL, STR_PRESSANYKEYTOSKIP,
                last_bad_switches == 0xff ? AU_SWITCH_ALERT : AU_NONE);
    uint8_t x = 2;
    for (uint8_t i = 0; i < NUM_SWITCHES - 1; i++) {
      ...
      if (i == 0)
        ...
        attr = (states & (1 << (i + 1))) == (switches_states & (1 << (i + 1)))
                   ? 0
                   : INVERS;
      if (!(g_model.switchWarningEnable & (1 << i)))
    ...
    for (uint8_t fm = 0; fm < MAX_FLIGHT_MODES; fm++) {
      ...
      for (uint8_t i = 0; i < MAX_LOGICAL_SWITCHES; i++) {
        ...
        if (ls->func == LS_FUNC_TIMER) {
          ...
          ls_sticky_struct &lastValue =
              (ls_sticky_struct &)LS_LAST_VALUE(fm, i);
          ...
          // if this ls was reset by the logicalSwitchesReset() the lastValue
          // will be set to CS_LAST_VALUE_INIT(0x8000) when it is unpacked into
          // ls_stay_struct the lastValue.duration will have a value of 0x4000
          // this will produce an instant true for edge logical switch if the
          // second parameter is big enough. So we reset it here.
          ...
          if (state) {
            ...
            if (lastValue.duration < 1000) lastValue.duration++;
          } else {
            if (lastValue.duration > lswTimerValue(ls->v2) &&
                (ls->v3 == 0 ||
                 lastValue.duration <= lswTimerValue(ls->v2 + ls->v3)))
              lastValue.state = true;
            lastValue.duration = 0;
          }
        }
  ...
  LogicalSwitchData *lswAddress(uint8_t idx) { return &g_model.logicalSw[idx]; }
  ...
  int16_t lswTimerValue(delayval_t val) {
    return (val < -109 ? 129 + val
                       : (val < 7 ? (113 + val) * 5 : (53 + val) * 10));
  }
  ...
  if (lswFamily(ls->func) == LS_FAMILY_OFS)
    ...
    val =
        convert8bitsTelemValue(ls->v1 - MIXSRC_FIRST_TELEM + 1, 128 + ls->v2) -
        convert8bitsTelemValue(ls->v1 - MIXSRC_FIRST_TELEM + 1, 128);
#endif
    return val;
  }
