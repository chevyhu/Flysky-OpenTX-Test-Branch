...
void battery_charge_init() {
  ...
  GPIO_InitStructure.GPIO_Pin =
      PWR_CHARGE_FINISHED_GPIO_PIN | PWR_CHARGING_GPIO_PIN;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_2MHz;
  GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;
  GPIO_Init(PWR_CHARGING_GPIO, &GPIO_InitStructure);
  GPIO_SetBits(PWR_CHARGING_GPIO,
               PWR_CHARGE_FINISHED_GPIO_PIN | PWR_CHARGING_GPIO_PIN);
}
...
uint16_t get_battery_charge_state() {
  ...
  if (!READ_CHARGE_FINISHED_STATE()) {
    finishedTime++;
    if (finishedTime > 1000) finishedTime = 1000;
    if (chargingTime) chargingTime--;
  } else if (!READ_CHARGING_STATE()) {
    chargingTime++;
    if (chargingTime > 1000) chargingTime = 1000;
...
void drawChargingInfo(uint16_t chargeState) {
  ...
  static int battery_top = (LCD_H - BATTERY_H) / 2,
             battery_left = (LCD_W - BATTERY_W) / 2;
  const char* text =
  ...
  if (progress >= 100) {
    progress = 0;
    battery_top = rand() % (LCD_H - BATTERY_H - BATTERY_CONNECTOR_H) +
                  BATTERY_CONNECTOR_H;
  ...
  lcd->drawSizedText(LCD_W / 2, text_top, text, strlen(text),
                     CENTERED | TEXT_BGCOLOR);

  lcd->drawFilledRect(battery_left, battery_top, BATTERY_W, BATTERY_H, SOLID,
                      ROUND | TEXT_BGCOLOR);
  lcd->drawFilledRect(battery_left + BATTERY_BORDER,
                      battery_top + BATTERY_BORDER, BATTERY_W_INNER,
                      BATTERY_H_INNER, SOLID, ROUND | TEXT_COLOR);

  lcd->drawFilledRect(battery_left + BATTERY_BORDER,
                      battery_top + BATTERY_BORDER + BATTERY_H_INNER - h,
                      BATTERY_W_INNER, h, SOLID, color);
  lcd->drawFilledRect(battery_left + (BATTERY_W - BATTERY_CONNECTOR_W) / 2,
                      battery_top - BATTERY_CONNECTOR_H, BATTERY_CONNECTOR_W,
                      BATTERY_CONNECTOR_H, SOLID, TEXT_BGCOLOR);
}
